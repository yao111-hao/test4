...
# 功能：实现结束后的后处理，根据目标是 Zynq 还是纯 FPGA
...
# 没有仿真参数
# 解析命令行参数
# 功能：遍历传入的 $argv，将合法的 -xxx value 更新到对应数组，并在控制台打印提示；未知参数则跳过。
...
# 展开数组为独立变量 & 检查参数合法性
# 将数组 -board_repo design_params 展开为变量 board_repo，去掉前导 -。
...
# Sanity check
# 随后对 min_pkt_len、max_pkt_len、num_queue 等做合法性检查，不符则 puts 提示并 exit。
...
# 加载板卡特定设置
# 根据 -board 参数加载对应的板卡 TCL 脚本，设置 part、board_part、zynq_family 等
source ${script_dir}/board_settings/${board}.tcl

#准备与切换到构建目录
# 若目录已存在，根据 -rebuild 和 -overwrite 行为选择保留、询问删除或直接更新。
# 最终创建或复用 build/${build_name} 目录，并在其中写入当前设计参数到 DESIGN_PARAMETERS 文件。
...
# Enumerate modules
# 枚举源码模块
# 遍历 src/ 下面的子目录，除了 shell，将每个模块名及路径存入 module_dict数组中
...
# 管理IP
# 功能：创建或打开 “Manage IP” 项目，用于独立综合各个 IP。
...
# 逐个生成/更新 IP
# 功能：对每个模块中的 IP 进行 out-of-context 综合，并根据 -synth_ip 决定是否实际运行综合流程。
...
dict for {module module_dir} $module_dict {
    ...
    foreach ip $ips {
        ...
        # 检查旧 IP 文件，必要时删除
        # 根据 overwrite 选项跳过或重建
        # source 模块/板卡 特定 IP 脚本
...
# 创建主设计项目、读取 IP
# 功能：建立顶层设计项目，设置目标器件、语言、宏定义，然后把先前综合好的 IP 插入项目。
...
# 定义 Verilog 宏：__synthesis__、板卡名宏 (可选 __zynq_family__,U50 U200 U250等)
...
# Import HBM subsystem block design (design_1) if provided
...
if {[file exists $hbm_bd_tcl]} {
    ...
    # Ensure the BD is saved and its output products are generated
    if {[llength [get_bd_designs -quiet]]} {
        save_bd_design
        set bd_name [current_bd_design]
        set bd_file [get_files -quiet ${bd_name}.bd]
        if {[llength $bd_file]} {
            generate_target all $bd_file
            # Sync generated files into the project without creating extra scripts
...
# 加载用户插件（P2P 等）
# 功能：针对不同频率的 “box” 插件，加载交叉开关脚本和地址映射，执行其自定义构建脚本，最后将插件路径加入 include_dirs。
...
# 读取所有模块的 RTL/VHDL 文件
# 功能：遍历模块与顶层目录，将所有 RTL/VHDL 文件导入到当前项目。
...
# 设置设计通用参数（Generic）与 Top
# 功能：把 design_params 中的参数转换为 uppercase generic，注入到综合／实现时的 generic 中，并指定顶层模块。
# Set vivado generic
...
# 动态生成一个 XDC，设置 user id 与 usr access，然后一并读取所有约束文件。
...
# 可选实现与后处理
# 功能：
# -impl 1 时执行实现并生成 bitstream
# -post_impl 1 时调用前面定义的后处理过程，生成 .mcs 或 .xsa
if {$impl} {
